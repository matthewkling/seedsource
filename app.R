...
d <- readRDS("../assets/seedsource/species_data.rds")
rsoil <- stack("../assets/seedsource/soil.tif")
clim1 <- stack("../assets/seedsource/clim1.tif")
clim2 <- stack("../assets/seedsource/clim2.tif")
clim3 <- stack("../assets/seedsource/clim3.tif")
...
ui <- fluidPage(
      ...
      fluidRow(
            column(3, selectizeInput("sp", "Focal\nspecies", d$spp)),
            #column(2, selectInput("year", "\nYear", c(1980, 2050, 2100))),
            ...
            column(3, selectInput("color_var", "point color", vars$description, "Environmental dissimilarity"))
      ),
...
server <- function(input, output) {
      ...
      data <- reactive({
            ...
            sc <- list(extract(clim1, site$point),
                       extract(clim2, site$point),
                       extract(clim3, site$point))
            #lapply(d$clim, extract, y=site$point)
            sc <- lapply(sc, function(x){colnames(x) <- colnames(jc); return(x)})
            ...
            if(input$pclim != 50){
                  ...
                  #jc <- jc * input$pclim / 50
                  #js <- js * (100 - input$pclim) / 50
            }
      ...
      output$map <- renderPlot({
            ...
            ggplot() + 
                  ...
                  geom_point(data=data()$jd, aes_string("longitude", "latitude", color=col_var())) +
      ...
      output$dist_scatter <- renderPlot({
            ggplot() + 
                  geom_point(data=data()$jd, aes_string("geo_dist", "env_dist_t1", color=col_var())) +
      ...
      output$env_scatter <- renderPlot({
            ...
            ggplot() + 
                  geom_point(data=data()$jd, aes_string(env_xvar(), env_yvar(), color=col_var())) +
                  geom_point(data=brushed$data, aes_string(env_xvar(), env_yvar()), color="orange") +
                  geom_path(data=se, aes_string(env_xvar(), env_yvar()), color="magenta",
                            arrow=arrow(type="closed", angle=15)) +
                  geom_point(data=se, aes_string(env_xvar(), env_yvar()), color="magenta", shape=10, size=5) +
                  scale_color_viridis_c() +
                  theme_minimal() +
                  theme(legend.position="none") +
